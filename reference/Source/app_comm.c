...
{
  while(!PABXCOMMConnected)
    {
      if (!LOOP_Main)
        {
          app_syslog( LOG_CRIT,"%s->%s(){Fallha na comunicacao com o PABX!}", __THIS_FILE__);
...
// boost_priority(THREAD_PRIO_PABXCOM);
  ...
  app_syslog(LOG_INFO, "%s->%s(){Waiting connection to PABXCOMM}",__THIS_FILE__ ); //"Aguardando conexão com PABXCOMM"
  InitPABXCOMM();		//Inicia socket de comunicação com processo ROUTER
//  FrmAPPVersionSEND();

  PABXCOMMConnected = TRUE;
...
//     if ( aplicacao == ADDR_ICIP_BLF_CTRL )
 ...
 app_syslog(LOG_CRIT, "%s->%s(){thread communication - FINISH!}",__THIS_FILE__ );
 PABXCOMMConnected = FALSE;
...
void InitPABXCOMM(void)
{
  ...
  app_syslog(LOG_WARNING, "%s->%s(){Start... }", __THIS_FILE__ );
